...
tPrData computeStatistics(CLASSES current_class, const vector<tGroundtruth> &gt,
        ...
        METRIC metric, bool compute_aos=false, double thresh=0, bool debug=false, FILE *report=NULL){
  ...
  for(int32_t i=0; i<gt.size(); i++){
    ...
    if(valid_detection==NO_DETECTION && ignored_gt[i]==0) {
      ...
      if (report) fprintf(report,"fn gt %d\n", i);
    }
    ...
    // only evaluate valid ground truth <=> detection assignments (considering difficulty level)
      ...
      if (report) fprintf(report,"ok gt %d det %d\n", i, det_idx);
    }
    // found a valid true positive
      ...
      if (report) fprintf(report,"tp gt %d det %d\n", i, det_idx);
    }
  }
  ...
  if(compute_fp){
    ...
    for(int32_t i=0; i<det.size(); i++){
      ...
      if(!(assigned_detection[i] || ignored_det[i]==-1 || ignored_det[i]==1 || ignored_threshold[i])) {
        ...
        if (report) fprintf(report,"fp det %d\n", i);
      }
    }
...
bool eval_class (FILE *fp_det, FILE *fp_ori, CLASSES current_class,
  ...
  // ------> try record which are right and wrong (paul kwon)
  if ((difficulty==MODERATE) && (metric==IMAGE))
  {
      cout << "reporting.." << endl;
      //cout << string("reporting..") + string("asd") + string(("%d", 3)) << endl;
      for (int32_t i=0; i<groundtruth.size(); i++){

        char fname[200];
        sprintf(fname, "%s/v%05d.txt", report_dir.c_str(), i);
        FILE *fp = fopen(fname, "w");

        tPrData dummy = tPrData();
        dummy = computeStatistics(current_class, groundtruth[i], detections[i],  dontcare[i], ignored_gt[i], ignored_det[i], true, boxoverlap, metric, false, 0, false, fp);
        fclose(fp);
      }
  }

  // <------try record which are right and wrong
